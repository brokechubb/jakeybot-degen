...
class Engagement(commands.Cog):
    ...
    def _load_config(self):
        ...
        try:
            if os.path.exists(config_path):
                ...
                logging.warning(f"Engagement config file not found at {config_path}, using defaults")
        ...
        try:
            ...
            async for doc in collection.find({}):
                ...
                if guild_id and channel_id:
                    ...
                    logging.info(f"Loaded persistent engagement for guild {guild_id} in channel {channel_id}")
            
            logging.info(f"Loaded {len(self.active_channels)} persistent engagement settings")
        except Exception as e:
        ...
        try:
            ...
            if action == "engage":
                ...
                if self.config["engagement"]["enable_logging"]:
                    logging.info(f"Saved engagement setting for guild {guild_id} in channel {channel_id}")
    ...
    async def _perform_cleanup(self):
        ...
        if channels_to_remove:
            logging.info(f"Cleaned up {len(channels_to_remove)} invalid engagement settings")
    ...
    async def _interject_in_channel(self, channel: discord.TextChannel):
        ...
        if random.random() < self.config["engagement"]["interjection_probability"]:
            try:
                message = random.choice(self.config.get("messages", self._get_default_messages()))
                ...
                logging.warning(f"Jakey doesn't have permission to send messages in #{channel.name}")
            except Exception as e:
                logging.error(f"Failed to send engagement message in #{channel.name}: {e}")
    ...
    async def jakey_engage(self, ctx: discord.ApplicationContext):
        ...
        if len(self.active_channels) >= self.config["engagement"]["max_channels"]:
            await ctx.respond("Jakey is already engaging in the maximum number of channels. Disengage from another server first.", ephemeral=True)
            return
    ...
    @commands.slash_command(name="jakey_engagement_status", description="Check Jakey's current engagement status.")
    async def jakey_engagement_status(self, ctx: discord.ApplicationContext):
        if ctx.guild.id in self.active_channels:
            ...
            if channel:
                await ctx.respond(f"Jakey is actively engaging in {channel.mention} in this server. 🎰", ephemeral=True)
            else:
                await ctx.respond("Jakey's engagement channel no longer exists. Use `/jakey_disengage` to clean up.", ephemeral=True)
        else:
            await ctx.respond("Jakey isn't actively engaging in any channel in this server.", ephemeral=True)

    @commands.slash_command(name="jakey_engagement_list", description="List all servers where Jakey is actively engaging.")
    @commands.has_permissions(manage_guild=True)
    async def jakey_engagement_list(self, ctx: discord.ApplicationContext):
        if not self.active_channels:
            await ctx.respond("Jakey isn't actively engaging in any servers right now.", ephemeral=True)
            return
    ...
    @commands.slash_command(name="jakey_engagement_stats", description="View engagement statistics and configuration.")
    @commands.has_permissions(manage_guild=True)
    async def jakey_engagement_stats(self, ctx: discord.ApplicationContext):
        ...
        embed.set_footer(text="Use /jakey_engagement_list to see all active engagements")
        await ctx.respond(embed=embed, ephemeral=True)
