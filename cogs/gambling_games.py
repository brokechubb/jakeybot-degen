...
class GamblingGames(commands.Cog):
    def __init__(self, bot):
        ...
        # Check if Gemini API is configured globally
        if hasattr(bot, "_gemini_api_configured") and bot._gemini_api_configured:
            self._gemini_api_configured = True
            logging.info("GamblingGames cog detected globally configured Gemini API.")
        else:
            self._gemini_api_configured = False
            logging.warning("Gemini API not configured globally; dynamic trivia question generation will be unavailable.")
    ...
    async def _generate_ai_questions(self, topic: str, rounds: int):
        ...
        Format the output as a JSON array of objects, where each object has 'question', 'options' (an array of strings), and 'answer' (the correct option letter, e.g., 'A').
    ...
    async def create_bet(self, ctx: discord.ApplicationContext, title: str, options: str, duration_minutes: int = 5):
        if ctx.guild.id in self.active_bets:
            await ctx.respond("There's already an active bet in this server. Finish or cancel it first.", ephemeral=True)
            return

        bet_options = [opt.strip() for opt in options.split(',')]
        if len(bet_options) < 2:
            await ctx.respond("You need at least two options for a bet.", ephemeral=True)
            return
        ...
        for i, option in enumerate(bet_options):
            if i < len(emojis):
                ...
                self.active_bets[ctx.guild.id]["options"][emoji] = {"option_text": option, "votes": 0}
            else:
                await ctx.respond("Too many options! Maximum 10 options allowed.", ephemeral=True)
                del self.active_bets[ctx.guild.id]
                return

        embed.add_field(name="Options", value=options_text, inline=False)
        embed.set_footer(text=f"Bet ends in {duration_minutes} minutes. Good luck, degens! 💀")
        ...
        self.bot.loop.create_task(self._end_bet_timer(ctx.guild.id, duration_minutes * 60))
    ...
    async def _end_bet(self, guild_id: int):
        ...
        for reaction in message.reactions:
            if str(reaction.emoji) in bet_data["options"]:
                bet_data["options"][str(reaction.emoji)]["votes"] = reaction.count - 1 # Subtract bot's own reaction

        results_text = "### Bet Results:\n"
        total_votes = 0
        for emoji, option_info in bet_data["options"].items():
            results_text += f"{emoji} {option_info['option_text']}: {option_info['votes']} votes\n"
        ...
        if total_votes == 0:
            ...
            for emoji, option_info in bet_data["options"].items():
                if option_info["votes"] > max_votes:
                    ...
                    winning_option += f" and {option_info['option_text']}" # Handle ties
    ...
    @commands.slash_command(name="cancel_bet", description="Cancel the current active betting pool.")
    @commands.has_permissions(manage_channels=True)
    async def cancel_bet(self, ctx: discord.ApplicationContext):
        if ctx.guild.id in self.active_bets:
            bet_data = self.active_bets.pop(ctx.guild.id)
            await ctx.respond(f"Bet '{bet_data['title']}' has been cancelled. Jakey don't refund, but y'all got lucky this time. 💀", ephemeral=False)
    ...
    @discord.option("category", description="Choose a category or type a topic for AI-generated questions.", required=False)
    @discord.option("rounds", description="Number of questions to play (default: 5).", min_value=1, max_value=10, default=5, required=False)
    async def trivia(self, ctx: discord.ApplicationContext, category: str = None, rounds: int = 5):
        if ctx.guild.id in self.active_trivia:
            await ctx.respond("There's already a trivia game active in this server. Finish or cancel it first.", ephemeral=True)
            return
        ...
        if category and category in self.trivia_questions:
            questions_to_play = random.sample(self.trivia_questions[category], min(rounds, len(self.trivia_questions[category])))
            logging.info(f"Starting static trivia in {ctx.guild.name} for category '{category}' with {len(questions_to_play)} rounds.")
        elif category:
            # AI-generated questions
            ai_questions, error_message = await self._generate_ai_questions(category, rounds)
            if error_message:
                await ctx.followup.send(error_message)
                return
            questions_to_play = ai_questions
            logging.info(f"Starting AI-generated trivia in {ctx.guild.name} for topic '{category}' with {len(questions_to_play)} rounds.")
        else:
            # Default to a random static category if no category is provided
            if not self.trivia_questions:
                await ctx.followup.send("Jakey ain't got no trivia questions loaded. Tell the admin to add some or try an AI topic! 💀")
                return
            random_category_name = random.choice(list(self.trivia_questions.keys()))
            questions_to_play = random.sample(self.trivia_questions[random_category_name], min(rounds, len(self.trivia_questions[random_category_name])))
            logging.info(f"Starting random static trivia in {ctx.guild.name} for category '{random_category_name}' with {len(questions_to_play)} rounds.")

        if not questions_to_play:
            await ctx.followup.send("Couldn't find or generate any trivia questions for that. Try a different category or topic. Rigged. 💀")
            return
        ...
        await ctx.followup.send(f"🧠 Trivia game starting in {self.active_trivia[ctx.guild.id]['category']} category with {len(questions_to_play)} rounds! Get ready, degens! 🔥")
    ...
    async def _run_trivia_game(self, guild_id: int):
        ...
        if not channel:
            logging.error(f"Channel {trivia_data['channel_id']} not found for trivia game.")
        ...
        for i in range(len(trivia_data["questions"])):
            ...
            options_text = "\n".join([f"{self.emojis[j]} {opt}" for j, opt in enumerate(question_data["options"])])
            ...
            trivia_data["timer_task"] = self.bot.loop.create_task(self._end_trivia_round_timer(guild_id, 20))
            
            try:
                await asyncio.wait_for(self._wait_for_correct_answer(guild_id), timeout=20)
            except asyncio.TimeoutError:
                await channel.send(f"⏰ Time's up for Round {i + 1}! No one got it this time. The answer was **{question_data['answer']}**.")
    ...
    async def _wait_for_correct_answer(self, guild_id: int):
        ...
        while self.active_trivia.get(guild_id) and self.active_trivia[guild_id]["correct_answer_user"] is None:
    ...
    async def on_reaction_add(self, reaction: discord.Reaction, user: discord.User):
        ...
        if guild_id in self.active_trivia and reaction.message.id == self.active_trivia[guild_id]["message_id"]:
            trivia_data = self.active_trivia[guild_id]
            ...
            try:
                ...
                if chosen_option_index < len(current_question["options"]):
                    ...
                    if str(reaction.emoji) == self.emojis[ord(current_question["answer"].upper()) - ord('A')]:
                        # Correct answer!
                        if trivia_data["timer_task"] and not trivia_data["timer_task"].done():
                            trivia_data["timer_task"].cancel() # Stop the timer

                        time_taken = asyncio.get_event_loop().time() - reaction.message.created_at.timestamp()
                        score_awarded = max(1, 20 - int(time_taken)) # Faster answers get more points

                        trivia_data["scores"][user.id] = trivia_data["scores"].get(user.id, 0) + score_awarded
                        trivia_data["correct_answer_user"] = user.id # Mark as answered

                        await reaction.message.channel.send(f"💰 {user.mention} got it! The answer was '{current_question['options'][chosen_option_index]}'. Awarded {score_awarded} points!")
                        logging.info(f"User {user.name} answered correctly in trivia round {trivia_data['current_round'] + 1}.")
                    else:
    ...
    async def _end_trivia_game(self, guild_id: int):
        ...
        channel = self.bot.get_channel(trivia_data["channel_id"])
        if not channel:
            logging.error(f"Channel {trivia_data['channel_id']} not found for ending trivia game.")
            return
        ...
        if not trivia_data["scores"]:
            ...
            sorted_scores = sorted(trivia_data["scores"].items(), key=lambda item: item[1], reverse=True)
    ...
    @commands.slash_command(name="cancel_trivia", description="Cancel the current active trivia game.")
    @commands.has_permissions(manage_channels=True)
    async def cancel_trivia(self, ctx: discord.ApplicationContext):
        if ctx.guild.id in self.active_trivia:
            ...
            if trivia_data["timer_task"] and not trivia_data["timer_task"].done():
                trivia_data["timer_task"].cancel()
            await ctx.respond(f"Trivia game has been cancelled. Jakey's bored anyway. 💀", ephemeral=False)
            logging.info(f"Trivia cancelled in {ctx.guild.name}")
        else:
            await ctx.respond("No active trivia game to cancel in this server.", ephemeral=True)

    @commands.slash_command(name="trivia_leaderboard", description="Show the all-time trivia leaderboard.")
    async def trivia_leaderboard(self, ctx: discord.ApplicationContext):
        await ctx.defer()
        if not self.DBConn:
            await ctx.followup.send("❌ Database connection not available for leaderboards.")
            return
        ...
        if not leaderboard_data:
            await ctx.followup.send("No one's on the leaderboard yet. Get gambling on some trivia! 🎰")
            return
        ...
        for i, entry in enumerate(leaderboard_data):
            ...
            if user:
                leaderboard_text += f"{i+1}. {user.mention}: {entry['score']} points\n"
            else:
