...
class Tool(ToolManifest):
    ...
    async def _tool_function(self, prompt: str, url_context: list[str] = None):
        ...
        # Generate an image
        _status = await fal_client.submit_async(_endpoint, arguments=_params)
                ...
                if response.status == 200:
                    ...
                    _msgID = await self.method_send(
                        file=discord.File(
                            ...
                            filename=f"generated_image_{datetime.datetime.now().strftime('%H_%M_%S_%m%d%Y_%s')}.png",
                        )
                    )
                    _url_aware.append(_msgID.attachments[0].url)
                else:
                    raise ValueError(
                        f"❌ Failed to download image from {_images}, status code: {response.status}"
                    )
